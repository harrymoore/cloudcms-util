...
module.exports = function() {
    ...
    var newTracingFeature = r.newTracingFeature = function(config) {
        return config._type = tracingFeatureDefinition()._qname;
    };

    var tracingFeatureDefinition = r.tracingFeatureDefinition = function() {
        return {
            "d:type": "feature",
            "_qname": "util:import-tracable"
        };
    };
     ...
     * 
     */
    var getGitanaConfig = r.getGitanaConfig = function(prompt, useCredentialsFile, gitanaFilePath, username, password)
    {
        var gitanaConfig = JSON.parse("" + fs.readFileSync(gitanaFilePath));
        if (useCredentialsFile) {
            ...
            var doPrompt = require('prompt-sync')({
                sigint: true
            });
            gitanaConfig.username = doPrompt('name: ');
            gitanaConfig.password = doPrompt.hide('password: ');
     ...
     * @returns {Array}
     */
    var findFiles = r.findFiles = function(dirPath, name, depth)
    {
        try {
            var paths = klawsync(dirPath, {
                nodir: true, 
                depthLimit: depth || 2,
                traverseAll: true,
                filter: function (filePath) {
                    // console.log(chalk.green(`${JSON.stringify(filePath, null, 4)}`));
                    return name === path.basename(filePath.path);
                }
            });
            ...
            // console.log(chalk.red("Folder not found: " + dirPath + ". error: " + error));
            return null;
        }
    };
    ...
    var findFilesExt = r.findFilesExt = function(dirPath, ext)
    {
        ...
        try {
            var allFiles = fsextra.readdirSyncRecursive(dirPath);
